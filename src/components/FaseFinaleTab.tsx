...
type FaseTipo = 'quarti' | 'semifinale' | 'finale' | 'terzo_posto'
...
export default function FaseFinaleTab({ torneo, squadre, gironi, partite, campi, giornate, torneoId, onPartiteChange }: Props) {
  ...
  function showMsg(t: string, type: 'ok'|'err' = 'ok') {
  ...
  async function generaAuto() {
    ...
    if (acc.length === 0) { showMsg('Nessun accoppiamento generabile. Inserisci prima i risultati dei gironi.', 'err'); setGenerando(false); return }

    const ultimaGiornata = giornate[giornate.length-1]?.id ?? null
    await sb.from('partite').delete().eq('torneo_id', torneoId).in('fase', ['quarti','semifinale','finale','terzo_posto'])

    const { data, error } = await sb.from('partite').insert(
      acc.map((a, i) => ({
        torneo_id: torneoId, squadra_casa_id: a.casa.id, squadra_ospite_id: a.ospite.id,
    ...
    if (error) showMsg('Errore: ' + error.message, 'err')
      ...
      onPartiteChange([...partite.filter(p => !['quarti','semifinale','finale','terzo_posto'].includes(p.fase)), ...(data as Partita[])])
      showMsg('Fase finale generata!')
    }
    setGenerando(false)
  }
  ...
  async function aggiungiPartita(fase: FaseTipo) {
    if (squadre.length < 2) { showMsg('Aggiungi almeno 2 squadre', 'err'); return }
    const sb = createClient()
    const giornataDefault = giornate[giornate.length-1]?.id ?? null
    const maxOrd = Math.max(0, ...partiteElim.map(p => p.ordine_calendario)) + 1
    const { data } = await sb.from('partite').insert({
      torneo_id: torneoId, squadra_casa_id: squadre[0].id, squadra_ospite_id: squadre[1].id,
      fase, girone: null, girone_id: null, giocata: false,
      ordine_calendario: maxOrd, giornata_id: giornataDefault
    }).select('*, squadra_casa:squadre!squadra_casa_id(*), squadra_ospite:squadre!squadra_ospite_id(*), campo:campi(*)')
      .single()
    if (data) onPartiteChange([...partite, data as Partita])
  }
  ...
  const fasi: { key: FaseTipo; label: string }[] = [
    { key: 'quarti', label: 'Quarti di finale' },
    { key: 'semifinale', label: 'Semifinali' },
    { key: 'finale', label: 'Finale' },
    { key: 'terzo_posto', label: '3°/4° posto' },
  ]

  return (
    <div className="space-y-4">
      {msg && (
        <div className={`px-4 py-2 rounded-lg text-sm ${msgType==='err' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>{msg}</div>
      )}

      {/* Info e azioni */}
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-800">
        <p>Genera automaticamente gli accoppiamenti dalle classifiche, oppure configurali manualmente per ogni fase.</p>
        {gironi.length === 0 && <p className="mt-1 text-amber-700 font-medium">⚠️ Nessun girone configurato — gli accoppiamenti saranno basati sull'ordine delle squadre.</p>}
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <button onClick={generaAuto} disabled={generando}
          ...
          {generando ? 'Generando...' : '⚡ Genera automaticamente dalle classifiche'}
        </button>
      </div>

      {/* Accoppiamenti per fase */}
      {fasi.map(({ key, label }) => {
        const pf = partiteElim.filter(p => p.fase === key)
        const visibile = key !== 'quarti' || (torneo.n_squadre_eliminatoria ?? 4) >= 8
        const terzoPostoVisible = key === 'terzo_posto' && torneo.finale_terzo_posto
        if (key === 'quarti' && !visibile) return null
        if (key === 'terzo_posto' && !terzoPostoVisible) return null
        ...
        return (
          <div key={key} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 flex items-center justify-between border-b border-gray-100"
              ...
              <button onClick={() => aggiungiPartita(key)}
            ...
            {pf.length === 0
              ...
              : pf.map(p => (
                <AccoppiamentoRow key={p.id} p={p} squadre={squadre} campi={campi} giornate={giornate}
                  primary={primary}
                  onUpdate={(updates) => aggiornaPartita(p.id, updates)}
...
function AccoppiamentoRow({ p, squadre, campi, giornate, primary, onUpdate, onDelete }: {
  ...
  async function save() {
    ...
    const updates: Partial<Partita> = {
      squadra_casa_id: casa, squadra_ospite_id: ospite,
      campo_id: campo || null, giornata_id: giornata || null,
      data_ora: dataOra || null
    }
    onUpdate(updates)
    setSaving(false)
  }
    ...
    <div className="px-4 py-3 border-b border-gray-50 last:border-0">
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        ...
        <div>
          ...
          <select value={ospite} onChange={e => setOspite(e.target.value)}
            className="w-full px-2 py-1.5 border border-gray-300 rounded text-xs">
            {squadre.map(s => <option key={s.id} value={s.id}>{s.nome}</option>)}
          </select>
        </div>
        ...
        <div>
          ...
          <select value={giornata} onChange={e => setGiornata(e.target.value)}
            className="w-full px-2 py-1.5 border border-gray-300 rounded text-xs">
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <button onClick={onDelete}
          className="px-3 py-1.5 border border-red-200 text-red-500 rounded text-xs hover:bg-red-50">
          Elimina
        </button>
      </div>
    </div>
